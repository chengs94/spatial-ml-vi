...
spatRF_trntst <- function( pars, Y, X, coords, t = 500, cov.type = "exp", 
                           cov.opts = NULL, Xtest = NULL, 
                           coords.test = NULL, m = floor(dim(X)[2]/3),  r = 5, 
                           k = floor(length(Y)/r), replace = FALSE,
                           s = ifelse (replace,length(Y), 
  ...
  if (is.null(coords.test)) coords.test = coords
  loo = is.null(Xtest)
  ...
  # if(!is.null(Xtest)) {
    qts = c(.25, .5, .75)
    col.idx = 1:ncol(X) # 1:ncol(Xtest)
    n.test <- ifelse(!is.null(Xtest), dim(Xtest)[1], dim(X)[1])
    Ytest <- ftest <- ztest <- matrix(0, nrow = n.test, 
                                      ncol = length(qts)*ncol(X))#rep(0,n.test)
  # }
    ...
    if( !loo ) {
      ...
      for (l in col.idx){
        for (j in 1:length(qts)){
          Xtest = Xtest0
          Xtest[,l] = quantile(Xtest0[,l], qts[j])
          C.vldt <- .makeCmat(mod,Xtest)
          ftest[,kk] <- ftest[,kk] + C.vldt %*% mod$muk 
          
          if(cov.type=="TPRS"){
            ztest[,kk] <- ztest[,kk] + X.vldt %*% mod$b +
              globalSpatBas$sig.test %*% globalSpatBas$i.sig %*% 
              (Y.train - cbind(mod$Ck,mod$X) %*% c(mod$muk,mod$b))
          } else{
            ztest[,kk] <- ztest[,kk] + 
      ...
      for(ii in row.idx){
        coords.test = matrix(coords[ii,], nrow = 1)
        
        if (!ii %in% train){
          globalSpatBas <- try(.makeSpatBas(pars,coords.train=coords[train,],
                                        coords.test = coords.test,
                                        cov.type=cov.type,cov.opts = cov.opts))
          
          if (class(globalSpatBas) != 'try-error'){
            kk = 1
            for (l in col.idx){
              for (j in 1:length(qts)){
                Xtest = matrix(Xtest0[ii,], nrow = 1)
                Xtest[,l] = quantile(Xtest0[,l], qts[j])
                C.vldt <- .makeCmat(mod,Xtest)
                ftest[ii,kk] <- ftest[ii,kk] + C.vldt %*% mod$muk 
                
                if(cov.type=="TPRS"){
                  ztest[ii,kk] <- ztest[ii,kk] + X.vldt[ii,] %*% mod$b +
                    globalSpatBas$sig.test %*% globalSpatBas$i.sig %*% 
                    (Y.train - cbind(mod$Ck,mod$X) %*% c(mod$muk,mod$b))
                } else{
                  ztest[ii,kk] <- ztest[ii,kk] + 
                    globalSpatBas$sig.test %*% globalSpatBas$i.sig %*% 
                    (Y.train - mod$Ck %*% mod$muk) 
                }
                kk = kk + 1
            ...
            t.specified[ii] = t.specified[ii] - 1
          }
        } else{
          
          # idx2.trn = setdiff(train, ii)
          idx2.trn = which(train == ii)
          globalSpatBas <- try(.makeSpatBas(pars,coords.train=coords[train[-idx2.trn],],
                                        coords.test = coords.test,
                                        cov.type=cov.type,cov.opts = cov.opts))
            ...
            for (l in col.idx){
              for (j in 1:length(qts)){
                ...
                if(cov.type=="TPRS"){
                  ztest[ii,kk] <- ztest[ii,kk] + X.vldt[ii,] %*% mod$b +
                    globalSpatBas$sig.test %*% globalSpatBas$i.sig %*% 
                    (Y.train[-idx2.trn] - cbind(mod$Ck[-idx2.trn,],mod$X[-idx2.trn,]) %*% c(mod$muk,mod$b))
                } else{
                  ztest[ii,kk] <- ztest[ii,kk] + 
                    globalSpatBas$sig.test %*% globalSpatBas$i.sig %*% 
                    (Y.train[-idx.trn2] - mod$Ck[-idx.trn2,] %*% mod$muk) 
                }
                kk = kk + 1
              }
            }
  ...
  # if(!is.null(Xtest)) {
    out$ftest <- ftest/t.specified
    out$ztest <- ztest/t.specified
  # }
