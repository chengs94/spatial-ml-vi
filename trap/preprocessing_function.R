...
covariate.preprocess <- function( covars.mon, covars.sub = NULL, region, common=T,
...
)
{
  ...
  ####
  # extra.vars = unlist(exclude.vars)[which(unlist(exclude.vars) %in% colnames(dat))]
  # extra.vars = unlist(exclude.vars)[which(unlist(exclude.vars) %in% colnames(cov_grid))]
  # include.vars = c(include.vars, extra.vars)
