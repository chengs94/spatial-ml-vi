...
X.test = as.matrix(X.test[idx_subset,])
...
grid_pred = function(annavg, numComp, PLpar, CVpar, X.train, X.test){
  ...
  message("one round done")
  return(out)

}

# col_subset = c(1, 19, 21, 33, 37, 82, 135, 144, 153, 164, 174, 165)
col_subset = 1:ncol(X.test)
# qts = c(.25, .5, .75)
qts = c(.125, .25, .375, .5, .625, .75, .875)
...
# pred.diff = mclapply(X_list, FUN = grid_pred, mc.cores = numCores,
#                      annavg = dat$ufp_uw, numComp = bestNumComp[2],
#                      PLpar = bestPLpar[2,], CVpar = bestCVpar[2,],
#                      X.train = as.matrix(X))
...
bc.pred.diff = mclapply(X_list, FUN = grid_pred, mc.cores = numCores,
                     annavg = dat$bc_uw, numComp = bestNumComp[4],
                     PLpar = bestPLpar[4,], CVpar = bestCVpar[4,],
                     X.train = as.matrix(X))

save.image("bc_pred_cohort_median_diff.RData")

cov_cohort = cov_cohort[c("longitude", "latitude")]
cov_cohort = cov_cohort[idx_subset,]

save(grid.bc.uw, bc.pred.diff, # cov_cohort, idx_subset, 
     file = "bc_varimp_cohort_diff.RData")
