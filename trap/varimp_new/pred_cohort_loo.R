# variable importance assessed via leave-one-out prediction on MM data
...
grid_pred = function(annavg, numComp, PLpar, CVpar, X.train){
  Y.train = annavg
  X.test = X.train
  n.test = nrow(X.test)
  n.train = nrow(X)
  ...
  dmat <- dtestmat <- as.matrix(SpatioTemporal::crossDist(train.coords))
  ...
  spatRF.cv <- spatRF_trntst( pars=list(psill=CVpar[1],nugget=1-CVpar[1]), 
...
rslt.bc.uw = grid_pred(annavg = dat$bc_uw, numComp = bestNumComp[4],
                       PLpar = bestPLpar[4,], CVpar = bestCVpar[4,],
                       X.train = as.matrix(X))

save(rslt.bc.uw, file = "varimp_loo_bc_updated.RData")
# save(rslt.bc.uw, file = "bc_varimp_loo.RData")
# save.image("bc_varimp_loo_full.RData")

rslt.ufp.uw = grid_pred(annavg = dat$ufp_uw, numComp = bestNumComp[2],
                       PLpar = bestPLpar[2,], CVpar = bestCVpar[2,],
                       X.train = as.matrix(X))

save(rslt.ufp.uw, rslt.bc.uw, file = "varimp_loo_ufp_bc_updated.RData")
save.image("varimp_loo_ufp_bc_full_updated.RData")

# rslt.pm25.uw = grid_pred(annavg = dat$pm25_uw, numComp = bestNumComp[7],
#                        PLpar = bestPLpar[7,], CVpar = bestCVpar[7,],
#                        X.train = as.matrix(X))
# 
# 
# save(rslt.pm25.uw, file = "varimp_loo_pm25.RData")
# save.image("varimp_loo_pm25_full.RData")