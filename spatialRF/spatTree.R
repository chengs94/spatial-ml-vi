...
cvSpatRF <- function(Y,X,coords,cov.type="exp", cov.opts = list(range=NULL),
                      cv.lambda = seq(0.05,0.95,0.05),lklhd=FALSE, ...) {
  ...
  for(lambda in cv.lambda){
    spatRF.obj <- spatRF( pars=list(psill=lambda,nugget=1-lambda), Y=Y, X=X, 
    ...
    if(lklhd){
      ...
      lklhd.vals[which(names(lklhd.vals)==lambda)] <-  cur.lklhd
    }
